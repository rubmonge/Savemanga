 ...
 * @category   Savemanga
 * @package    Savemanga_Sekaimaga
 * @author     Rubén Monge <rubenmonge@gmail.com>
 * @copyright  Copyright (c) 2011-2012 Rubén Monge. (http://www.rubenmonge.es/)
 */
class Savemanga_Sekaimanga extends Savemanga {
    /*
     * pattern manga: http://www.sekaimanga.net/Sekai/read-bleach-chapter-672-ID-25601-page-2.html : 
     * Where "bleach" == manga identifier (id)
     * "672" = episode
     * "2" = page
     */

    public function getManga($url) {
        $pageContent = $this->file_get_contents_curl($url, $url);
        if (strlen($pageContent)) {
            ...
            foreach ($options as $option) {
                ...
                $links[] = "http://www.sekaimanga.net/Sekai/" . $value;
            }
            ...
            foreach ($links as $k => $url) {
                ...
                var_dump($url);
                $imgpatter = "/<img (.*) class='chapter-img'/";
    ...
    public function setMangaID($url) {
        $aux = str_replace("http://www.sekaimanga.net/Sekai/", "", $url);
        $aux = str_replace(".html", "", $aux);
        $this->id = $aux;
    }
    ...
    final protected function saveImages() {
        ...
        if (is_array($this->images)) {
            ...
            $this->write("<br/>No se han encontrado imágenes o Sekaimaga.com ha tardado demasiado en contestar");
            return false;
        }
    }
