...
class Savemanga_Viewcomic extends Savemanga
{


	protected $_pattern = "https://viewcomic.com/oblivion-song-007-2018/";
	...
	protected $_domain = "https://viewcomic.com";
	...
	public function getManga($url)
	{
		...
		if ($dom) {
			...
			$aux = $dom->query('//div[contains(@class,"separator")]/*/img');
	...
	protected function setMangaNameAndEp($dom)
	{
		...
		$auxMangaNameA = trim($aux[0]->nodeValue);

		$aux = $dom->query('//h1');
		$auxMangaNameB = $aux[0]->nodeValue;

		if (strpos($auxMangaNameB, $auxMangaNameA)) {
			/* auxMangaNameB puede contener el episodio */
			$this->manga_name = $auxMangaNameA;
			$ep = explode("(", $auxMangaNameB);
			$this->manga_ep = (int)trim(str_replace(['–', '…', $this->manga_name], ["-", "", ""], $ep[0]));
			if (is_numeric($this->manga_ep)) {
				if ($this->manga_ep < 10) {
				...
				$this->manga_name = str_replace(" ", "_", ucwords($this->manga_name));
				$this->file_manga_name = $this->manga_name . ".cbr";
			}
		} else {
			/* seguramente el auxMangaNameB sería el correcto */
			$this->manga_name = ucwords(str_replace(['–', '…', " "], ["-", "", "_"], $auxMangaNameB));
			$this->file_manga_name = $this->manga_name . ".cbr";
		}
		return true;
	}
